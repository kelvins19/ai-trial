...
class VolunteerMatchSummary(BaseModel):
    volunteer_id: int
    matches: List[ActivityMatchResult]

class ActivityMatchSummary(BaseModel):
    activity_id: int
    activity_name: str
...
class VolunteerMatchingResponse(BaseModel):
    status: str
    message: str
    results: List[VolunteerMatchSummary]

class ActivityMatchingResponse(BaseModel):
    status: str
    message: str
    results: List[ActivityMatchSummary]
...
class VolunteerMatcher:
    ...
    def recommend_volunteers(self, activity: Activity, volunteers: List[Volunteer], top_n: int = 5) -> List[tuple[Volunteer, Match]]:
        # Initialize list to store matches
        matches = []
        
        # Pre-filter volunteers based on hard requirements
        eligible_volunteers = [
            v for v in volunteers
            if (
                v.age >= activity.min_age and
                (not activity.requires_background_check or v.background_check) and
                (not activity.requires_driving_license or v.has_driving_license) and
                v.experience_years >= activity.min_experience_years
            )
        ]
        
        # Analyze matches for eligible volunteers
        for volunteer in eligible_volunteers:
            match = self.analyze_match(volunteer, activity)
            matches.append((volunteer, match))
        
        # Sort matches by score in descending order
        sorted_matches = sorted(matches, key=lambda x: x[1].score, reverse=True)
...
def main():
    ...
    try:
        ...
        print("\nAnalyzing volunteer recommendations for activities...")
        for activity in activities:
            ...
            recommendations = matcher.recommend_volunteers(activity, volunteers)
            
            print("\nRecommended Volunteers:")
            for i, (volunteer, match) in enumerate(recommendations, 1):
                print(f"\n{i}. {volunteer.name}")
